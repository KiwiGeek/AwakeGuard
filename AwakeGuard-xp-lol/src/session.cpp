...
std::wstring ProtectionPhrase(const SessionState& session) {
    ...
    if (session.preventSleep) {
        parts.push_back(L"no sleep");
    }
    if (session.preventDisplayOff) {
        parts.push_back(L"no screensaver");
    }
    std::wstring result;
...
void ApplyScreensaverState(bool prevent) {
    if (prevent) {
        if (!g_screenSaveCaptured) {
            SystemParametersInfoW(SPI_GETSCREENSAVEACTIVE, 0, &g_savedScreenSaveActive, 0);
        ...
        SystemParametersInfoW(SPI_SETSCREENSAVEACTIVE, g_savedScreenSaveActive, nullptr, SPIF_SENDCHANGE);
...
std::wstring SessionStatusText(const SessionState& session) {
    if (!session.isActive) {
        return L"Off - normal power and lock behavior";
    }

    const std::wstring protection = ProtectionPhrase(session);
    if (!session.hasExpiry) {
        return L"On - " + protection + L" (until you turn off)";
    }

    const ULONGLONG remaining = session.expiresAtMs > NowMs() ? session.expiresAtMs - NowMs() : 0;
    if (remaining == 0) {
        return L"On - " + protection + L" (expiring)";
    }
    return L"On - " + protection + L" (" + FormatRemainingMs(remaining) + L" left)";
}
