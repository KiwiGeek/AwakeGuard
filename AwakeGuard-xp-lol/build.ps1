# Build AwakeGuard-XP-LOL with MSYS2 MinGW i686.
...
function Show-Msys2InstallHelp {
    Write-Host ""
    Write-Host "MSYS2 + MinGW 32-bit is required to cross-compile for Windows XP." -ForegroundColor Yellow
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "3) Install toolchain (all three packages):" -ForegroundColor Cyan
...
if (-not (Test-Path $cmakeExe)) {
    Show-Msys2InstallHelp
    Write-Error "cmake.exe not found in mingw32\bin. Install mingw-w64-i686-cmake in MSYS2 MinGW 32-bit."
}

$windresExe = Resolve-Windres $mingwBin
if (-not $windresExe) {
    Write-Host ""
    Write-Host "Missing windres.exe — install binutils for mingw32." -ForegroundColor Yellow
    Write-Host "In MSYS2 (any shell) run:" -ForegroundColor Cyan
    Write-Host "  pacman -S --needed --noconfirm mingw-w64-i686-binutils"
    Write-Host ""
...
if (-not $makeExe) {
    Write-Host ""
    Write-Host "Missing mingw32-make.exe — install the make package." -ForegroundColor Yellow
    Write-Host "In 'MSYS2 MinGW 32-bit' run:" -ForegroundColor Cyan
    Write-Host "  pacman -S --needed --noconfirm mingw-w64-i686-make"
    Write-Host ""
...
if ($Clean -and (Test-Path $buildDir)) {
    ...
    try {
        ...
        Write-Host "Could not delete build folder (file in use?). Close AwakeGuard-XP-LOL.exe or your IDE, then run: .\build.ps1 -Clean" -ForegroundColor Yellow
        throw
    }
}
