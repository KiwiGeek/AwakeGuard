...
StartupEnableResult TryRelocateAndEnable(HWND owner) {
    ...
    const std::wstring destinationExe = destinationDir + L"\\" + fs::path(sourceExe).filename().wstring();

    CreateDirectoryW(destinationDir.c_str(), nullptr);

    std::error_code ec;
    for (const auto& sourcePath : paths::RelocateArtifactPaths(sourceExe)) {
        const fs::path destinationPath = fs::path(destinationDir) / fs::path(sourcePath).filename();
        fs::copy_file(sourcePath, destinationPath, fs::copy_options::overwrite_existing, ec);
        if (ec) {
            const std::wstring message =
                L"Could not copy AwakeGuard:\n" + std::wstring(ec.message().begin(), ec.message().end());
